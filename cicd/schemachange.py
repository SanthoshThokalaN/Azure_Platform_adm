...
class JinjaTemplateProcessor:
  _env_args = {"undefined":jinja2.StrictUndefined,"autoescape":True, "extensions":[JinjaEnvVar]}
...
class SnowflakeSchemachangeSession:
  ...
  _q_ch_r_checksum = "SELECT DISTINCT SCRIPT, FIRST_VALUE(CHECKSUM) OVER (PARTITION BY SCRIPT " \
    ...
    + "STATUS = 'Success'"
  _q_ch_fetch ="SELECT VERSION, SCRIPT FROM {database_name}.{schema_name}.{table_name} WHERE SCRIPT_TYPE = 'V'"
...
#   print(_log_ch_max_version.format(max_published_version_display=max_published_version_display))
  ...
  for script_name in all_script_names_sorted:
    ...
    # Apply a versioned-change script only if the version does not exist in database.
    if script_name[0] == 'V' and script['script_version'] in change_history:
      if change_history[script['script_version']] != script_name:
        raise(ValueError(_err_dup_scripts_version.format(script_version=script['script_version'],
                                              script_full_path=script["script_full_path"])))
      scripts_skipped += 1
      continue
    ...
    if script_name[0] == 'R':
      ...
      if script_name in r_scripts_checksum:
...
def load_schemachange_config(config_file_path: str) -> Dict[str, Any]:
  ...
  if os.path.isfile(config_file_path):
    with open(config_file_path) as config_file:
      ...
      # has.
      config_template = jinja2.Template(config_file.read(), undefined=jinja2.StrictUndefined, autoescape=True, extensions=[JinjaEnvVar])
...
def get_schemachange_config(config_file_path, root_folder, modules_folder, snowflake_account, \
  ...
  cli_inputs = { "root_folder":root_folder, \
    ...
    "change_file_list": change_file_list}
...
def main(argv=sys.argv):
  ...
  parser_deploy.add_argument('-cfl', '--change-file-list', nargs='*', help = 'Comma delimited list of files that should be included in the deploy check. A scripts will process as normal.', required = False)
