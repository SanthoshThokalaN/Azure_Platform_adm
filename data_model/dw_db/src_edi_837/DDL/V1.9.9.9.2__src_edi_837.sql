...
SELECT DISTINCT   
  ...
  SUBSTR(clm_recpt_dt, REGEXP_INSTR(clm_recpt_dt, ''clm_recpt_dt='')+13, 10) as clm_recpt_dt,
...
,$43
,metadata$filename
from ''||:V_STAGE||'' ) file_format = ''''UTIL.FF_TILDA_CSV'''', pattern=''''.*dt_cmpltd=.*.000.*'''' ;''
;
...
SELECT DISTINCT   
  ...
  pat_para_num , 
  doc_ctl_nbr ,
  SUBSTR(dt_cmpltd, REGEXP_INSTR(dt_cmpltd, ''dt_cmpltd='')+10, 10) as dt_cmpltd,
