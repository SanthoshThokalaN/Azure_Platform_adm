...
CREATE OR REPLACE PROCEDURE "SP_PROF_SUBSCRIBER"("PIPELINE_ID" VARCHAR(16777216), "PIPELINE_NAME" VARCHAR(16777216), "DB_NAME" VARCHAR(16777216), "UTIL_SC" VARCHAR(16777216), "TGT_SC" VARCHAR(16777216), "SRC_EDI_837_SC" VARCHAR(16777216), "FILE_SOURCE" VARCHAR(16777216), "LAST_SUCCESSFUL_LOAD" VARCHAR(16777216), "WH" VARCHAR(16777216))
...
V_SUB_PROCESS_NAME VARCHAR DEFAULT ''PROF_SUBSCRIBER'';
...
ALTER SESSION SET TIMEZONE = ''America/Chicago'';
   ...
   V_STEP_NAME := ''LOAD PROF_SUBSCRIBER'';
...
INSERT INTO IDENTIFIER(:V_PROF_SUBSCRIBER)
(
  APP_SENDER_CODE , 
  APP_RECIEVER_CODE , 
  GRP_CONTROL_NO , 
...
AND NOT EXISTS ( SELECT 1 FROM  IDENTIFIER(:V_PROF_SUBSCRIBER) P WHERE P.FILE_SOURCE = :FILE_SOURCE AND R.XML_MD5 = P.XML_MD5)
...
XMLGET( SBR.value, ''subscriberInsuranceType'' ):"$"::VARCHAR  As subscriber_insurance_type,
null as subscriber_cob_code,
null as yes_no_response_code,
null as subscriber_employement_status,
XMLGET(SBR.value, ''clmFilingIndicatorCode'' ):"$"::VARCHAR  As claim_type
...
Loop2010BA_REF_SY AS 
(
select 
XML_MD5, 
  ''SY'' as SUBSCRIBER_SUPLEMENTAL_ID_QLFR,
XMLGET( REF_SY.value , ''subscriberSupplementalId'' ):"$"::INTEGER  As subscriber_sumplemental_id
...
where (Loop2320_Loop2330A_NM1.XML_MD5 is not null or Loop2320_Loop2330A_N3.XML_MD5 is not null or Loop2320_Loop2330A_N4.XML_MD5 is not null )
) 
...
----
Loop2010BA_REF_SY.SUBSCRIBER_SUPLEMENTAL_ID_QLFR,
Loop2010BA_REF_SY.subscriber_sumplemental_id,
...
FROM claimTrackingId 
     ...
     LEFT JOIN Loop2010BA_REF_SY on GS.XML_MD5 = Loop2010BA_REF_SY.XML_MD5    
