...
V_SUB_PROCESS_NAME VARCHAR DEFAULT ''INST_RECEIVER_NAME'';
...
V_INST_RECEIVER_NAME VARCHAR := :DB_NAME||''.''||COALESCE(:TGT_SC, ''SRC_EDI_837'')||''.INST_RECEIVER_NAME'';
...
ALTER SESSION SET TIMEZONE = ''America/Chicago'';
   ...
   V_STEP_NAME := ''LOAD INST_RECEIVER_NAME'';
...
INSERT INTO identifier(:V_INST_RECEIVER_NAME)
    (
  APP_SENDER_CODE , 
  APP_RECIEVER_CODE , 
  GRP_CONTROL_NO , 
...
WITH 
RAW AS 
(SELECT * FROM IDENTIFIER(:V_INST_CLAIMS_RAW) WHERE FILE_SOURCE = :FILE_SOURCE AND ISDC_LOAD_DT >= :LAST_SUCCESSFUL_LOAD
...
SELECT * FROM TEMP WHERE NOT EXISTS (SELECT 1 FROM IDENTIFIER(:V_INST_RECEIVER_NAME) P WHERE P.XML_HDR_MD5 = TEMP.XML_HDR_MD5)
